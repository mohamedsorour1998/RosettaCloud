...
**Fraud prevention — what is already running**: Amazon Cognito requires a confirmed email before any lab can launch. Redis enforces a single active lab per account — no parallel abuse. Every lab has a hard TTL and terminates automatically. Karpenter's node cap prevents unbounded provisioning even under coordinated pressure. Per-user rate limits on lab creation and AI requests are enforced at the API level, not the UI. Free-tier lab hours are tracked per calendar week in DynamoDB and enforced server-side. AI message usage is also quota-tracked per week in DynamoDB — 50 messages per free-tier user — enforced at the API gateway before any AgentCore call is placed, so a bad actor cannot exhaust AI inference budget by scripting requests. None of these are roadmap items. All seven are live.

The cost of a malicious actor farming free labs: they get two hours per week, one cluster at a time, 50 AI messages per week, confirmed by email. The abuse surface is small by design.
...
**On the business model**: The math is now explicit, and it covers the full cost. A free user costs approximately $0.40 per month to serve — $0.35 in lab compute (two hours per week at $0.04 per hour on spot t3.xlarge) plus roughly $0.05 in AI inference for 50 messages per week at Nova 2 Lite rates (~$0.00025 per message). A Pro subscriber at $19 per month, using an average of eight lab hours and around 200 AI tutor messages, costs approximately $0.37 total — generating $18.63 in gross margin (~98%). At steady state, one Pro subscriber subsidises approximately 46 free users. University bulk deals at $7-9 per student per month maintain similar margins at volume. The freemium model is standard for developer tools. The unit economics here are favourable because the variable cost is compute and inference, not people — and both scale predictably.
